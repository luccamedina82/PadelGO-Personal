...
export default function BookingQuickPopover({
  ...
  const pos = (() => {
    ...
    const estimatedHeight = isBlockSource(b.source) ? 200 : 340
    const offsetX = 18
    ...
    >
      ...
      {confirmingCancel && (
        <div className="px-3.5 py-2.5 flex flex-col gap-2" style={{ borderBottom: '1px solid var(--border)' }}>
          <p className="text-[12px] text-center text-muted">{isBlock ? '¿Cancelar este bloqueo?' : '¿Cancelar esta reserva?'}</p>
      ...
      {!confirmingCancel && (
        <div className="flex flex-col">
          {isBlock ? (
            /* Bloques: solo "Ver detalle" (sin WhatsApp) + cancelar si aplica */
            <div style={{ borderBottom: !isCancelled ? '1px solid var(--border)' : undefined }}>
              <button
                onClick={onOpenDetail}
                className="w-full flex items-center justify-center gap-1.5 py-2.5 text-[12px] font-medium text-muted hover:text-text hover:bg-card-hover transition-colors"
              >
                Ver detalle
            ...
            /* Reservas normales: WhatsApp + Ver detalle */
            <div className={`grid ${!isCancelled ? 'grid-cols-2' : 'grid-cols-1'}`} style={{ borderBottom: !isCancelled ? '1px solid var(--border)' : undefined }}>
              {phone ? (
                <a
                  ...
                  style={{ color: '#4dc870', borderRight: !isCancelled ? '1px solid var(--border)' : undefined }}
                >
              ...
              <button
                onClick={onOpenDetail}
                className="flex items-center justify-center gap-1.5 py-2.5 text-[12px] font-medium text-muted hover:text-text hover:bg-card-hover transition-colors col-span-1"
                style={isCancelled ? { gridColumn: '1 / -1' } : undefined}
              >
                Ver detalle
                <svg width="9" height="9" viewBox="0 0 10 10" fill="none">
                  <path d="M2 5h6M5 2l3 3-3 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
          ...
          {/* Cancelar (reservas y bloqueos no cancelados) */}
          {!isCancelled && (
            <button
              ...
              style={{ color: '#f87171' }}
            >
              {isBlock ? 'Cancelar bloqueo' : 'Cancelar reserva'}
            </button>
          )}
        </div>
      )}
